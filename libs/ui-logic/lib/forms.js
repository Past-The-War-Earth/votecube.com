...
export function ensureChildForm(parentFormName, childFormName, page, navigateToRouteOnNotFound, navigateParamsOnNotFound) {
...
export function ensureTopForm(topFormName, formHandle, navigateToRouteOnNotFound, navigateParamsOnNotFound) {
...
export function navigateOnValid(form, navigateToRouteOnValid, paramMap) {
    form.touch();
    if (!form.valid) {
        return;
    }
...
export var OPTIONS = {
    handleKeydown: (page, multi, event) => {
        if (!page.showOptions) {
            return;
        }
    ...
    showFiltered: (page, element, event
    // focus
    ) => {
        if (!page.showOptions) {
