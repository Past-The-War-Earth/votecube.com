<script>
	...
	let logicUtils;
	let minusStyles =
		"display: inline-block; position: absolute; left: 0px; top: 5.5px;";
	let plusStyles =
		"display: inline-block; position: absolute; right: 0px; top: 5.5px;";
	let redSlider;
	let select;
	...
	function matchColor(color, colorMap, select) {
...
{#if $forms && colors}
	<section>
		...
		<div>
			<select on:input={selectColor} bind:this={select}>
			...
			{#if trackOriginal}
				<UndoIcon multiLine="y" on:select={revert} />
			{/if}
			<br />
			<figure
