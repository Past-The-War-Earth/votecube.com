import type { IAgreementApi } from './wrappers/AgreementApiClient';
import type { IIdeaApi } from './wrappers/IdeaApiClient';
import type { IIdeaSituationApi } from './wrappers/IdeaSituationApiClient';
export declare const core: import("@airport/direction-indicator").IInjectionApplication;
export declare const AGREEMENT_API: import("@airport/direction-indicator").IDependencyInjectionToken<IAgreementApi>;
export declare const IDEA_API: import("@airport/direction-indicator").IDependencyInjectionToken<IIdeaApi>;
export declare const IDEA_SITUATION_API: import("@airport/direction-indicator").IDependencyInjectionToken<IIdeaSituationApi>;
//# sourceMappingURL=tokens.d.ts.map