export class UiObjectBuilder {
    ...
    setEach(callback, options) {
        const { lastToFragment, nestedFrom, nestedTo } = this.traversePath(
        // true
        );
        if (!options) {
            options = {};
    ...
    doSet(paths, callback, fromKey, options) {
        let { lastToFragment, nestedFrom, nestedTo } = paths;
        if (callback) {
            if (callback instanceof Function) {
                ...
                const nestedObjDelta = new UiObjectBuilder(nestedFrom, nestedTo);
    ...
    toUiObject(uiObject, uiKey, dbObject) {
