...
*/
    ...
    onMount(async () => {
        ...
        form = forms.getForm(forms.DEFINE_IDEA_TOP);
        ...
        if (!form) {
            ...
            try {
                ...
                forms.setForm(forms.DEFINE_IDEA_TOP, form);
            } catch (theError) {
                error = theError;
                return;
            }
    ...
    async function doSubmit($routeParams, modified) {
        ...
        forms.navigateOnValid(form, IDEA_MAIN, $routeParams);
    }
</script>

{#if form}
    <form>
        <legend>Define Idea</legend>
